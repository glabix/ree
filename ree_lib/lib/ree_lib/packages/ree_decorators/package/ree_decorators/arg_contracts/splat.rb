  ...
  module ArgContracts
    class Splat
      ...
      def initialize(*contracts)
        ...
        forbidden_class_contracts = Validators::FORBIDDEN_CONTRACTS
      ...
      def valid?(value)
        ...
        if @middle_splat_of
          ...
          # after splat part
          is_valid = value[value.size - right_contract_count..].each_with_index.all? do |value, idx|
      ...
      def message(value, name, lvl = 1)
        ...
        if @validators.size - 1 > value.size
          return "expected at least #{@validators.size - 1} #{pluralize(@validators.size, 'value', 'values')} for #{to_s}, got #{value.size} #{pluralize(value.size, 'value', 'values')} => #{truncate(value.inspect, 80)}"
        end
