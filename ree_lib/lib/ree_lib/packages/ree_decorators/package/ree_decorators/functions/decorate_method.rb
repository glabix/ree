...
class ReeDecorators::DecorateMethod
  ...
  def call(target, method_name, is_class_method)
    return if !target.instance_variable_defined?(ReeDecorators::DSL::TMP_DECORATORS_STORAGE_KEY)
    decorators = target.remove_instance_variable(ReeDecorators::DSL::TMP_DECORATORS_STORAGE_KEY)

    return if !decorators&.any?

    decorators_storage = target.instance_variable_get(ReeDecorators::DSL::DECORATORS_STORAGE_KEY)
    if decorators_storage.nil?
      decorators_storage = {}
      target.instance_variable_set(ReeDecorators::DSL::DECORATORS_STORAGE_KEY, decorators_storage)
    end
    ...
    alias_target.class_eval(<<~RUBY, file, line)
      #{method_visibility}def #{method_name}(*args, **kwargs, &blk)
        decorators = #{target.name}
          .instance_variable_get(ReeDecorators::DSL::DECORATORS_STORAGE_KEY)
