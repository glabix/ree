...
RSpec.describe "RubyLsp::Ree::HoverListener" do
  ...
  it "returns missing translation result for locale" do
    source =  <<~RUBY
      class SomeClass
        fn :some_class

        SomeError = invalid_param_error(:some_error_code, 'some_key.unexisting_key')
    ...
    with_server(source, sample_file_uri) do |server, uri|
      ...
      result = server.pop_response
      expect(result.response.contents.value).to match('MISSING TRANSLATION')
    end
  end

  # TODO it "returns missing translation result for error code"
end
