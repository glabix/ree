...
RSpec.describe "RubyLsp::Ree::ReeFormatter" do
  ...
  it "sorts links inside fn block by groups" do
    ...
    expect(result.lines[3].strip).to eq('link :linked_service_1, target: :both')
    expect(result.lines[4].strip).to eq('link :linked_service_3, from: :some_package')
    expect(result.lines[5].strip).to eq("link 'some/file/path', -> { SomeConst2 }")
    expect(result.lines[6].strip).to eq('import -> { SomeConst }, from: :some_package')
    expect(result.lines[7].strip).to eq('end')
  end
end
